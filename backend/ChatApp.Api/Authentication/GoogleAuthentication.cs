...
public static class GoogleAuthentication
{
    ...
    public static bool AddGoogleSignIn(this WebApplicationBuilder builder)
    {
        var googleClientId = builder.Configuration["Authentication:Google:ClientId"] ?? string.Empty;
        if (string.IsNullOrWhiteSpace(googleClientId)) return false;
        ...
        .AddJwtBearer(Scheme, options =>
        {
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                OnTokenValidated = async context =>
                {
                    ...
                    var db = context.HttpContext.RequestServices.GetRequiredService<ChatAppDbContext>();
                    await using var transaction = await db.Database.BeginTransactionAsync();
                    var userManager = context.HttpContext.RequestServices.GetRequiredService<UserManager<ChatUser>>();
                    ...
                    if (user is null)
                    {
                        ...
                        ActivityAudit.Add(db, "UserCreated", user, metadata: new { provider = "Google" });
                    ...
                    if (logins.All(x => x.LoginProvider != "Google" || x.ProviderKey != subject))
                    {
                        ...
                        ActivityAudit.Add(db, "ExternalLoginLinked", user, metadata: new { provider = "Google" });
                        await db.SaveChangesAsync();
                    }
    ...
    {
        app.MapPost("/api/auth/exchange/google", async (HttpContext context, UserManager<ChatUser> manager, JwtSessionService sessions) =>
        {
    ...
    private static string? NormalizeProfileName(string? value) => string.IsNullOrWhiteSpace(value) ? null : value.Trim()[..Math.Min(value.Trim().Length, 100)];
}
