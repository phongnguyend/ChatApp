...
public static class UserAdministration
{
    public static async Task<IResult> SaveAsync(Guid? id, ManageUserRequest request, ClaimsPrincipal principal,
        UserManager<ChatUser> manager, ChatAppDbContext db, CancellationToken ct)
    {
        ...
        var account = id.HasValue ? await manager.FindByIdAsync(id.Value.ToString()) : new ChatUser();
        if (account is null) return Results.NotFound();
        var wasEnabled = account.IsEnabled;
        var beforeProfile = new { account.Email, account.FirstName, account.LastName, account.PhoneNumber };
        ...
        if (id.HasValue && emailChanged && !string.IsNullOrEmpty(account.Email))
        ...
        if (!id.HasValue || string.IsNullOrEmpty(account.Email))
        {
            account.Email = email;
            if (!id.HasValue) account.UserName = email;
        }
        ...
        var afterProfile = new { account.Email, account.FirstName, account.LastName, account.PhoneNumber };
        if (!id.HasValue) ActivityAudit.Add(db, "UserCreated", account, principal, new { roles });
        else if (!beforeProfile.Equals(afterProfile)) ActivityAudit.Add(db, "UserUpdated", account, principal);
        ...
        if (id.HasValue && (removed.Length > 0 || added.Length > 0))
            ActivityAudit.Add(db, "RolesChanged", account, principal, new { before = existingRoles, after = roles });
        await db.SaveChangesAsync(ct);
        await transaction.CommitAsync(ct);
        return id.HasValue ? Results.NoContent() : Results.Created($"/api/admin/users/{account.Id}", new { account.Id });
    }
