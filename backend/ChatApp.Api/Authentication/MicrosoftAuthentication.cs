...
public static class MicrosoftAuthentication
{
    ...
    {
        app.MapPost("/api/auth/exchange/microsoft", (HttpContext context, UserManager<ChatUser> manager, ChatAppDbContext db, JwtSessionService sessions) =>
        ...
        app.MapPost("/api/auth/link/microsoft", async (HttpContext context, UserManager<ChatUser> manager, ChatAppDbContext db, JwtSessionService sessions) =>
        {
    ...
    private static async Task<IResult> Complete(HttpContext context, ClaimsPrincipal external, ChatUser? linkTo,
        UserManager<ChatUser> manager, ChatAppDbContext db, JwtSessionService sessions)
    {
        ...
        if (user is null)
        {
            ...
            ActivityAudit.Add(db, "UserCreated", user, metadata: new { provider = "Microsoft" });
        }
        ...
        if (!logins.Any(login => login.LoginProvider == "Microsoft" && login.ProviderKey == key))
            ActivityAudit.Add(db, "ExternalLoginLinked", user, linkTo is null ? null : context.User, new { provider = "Microsoft" });
        ...
        return Results.Ok(sessions.Issue(context, user, roles));
    }
