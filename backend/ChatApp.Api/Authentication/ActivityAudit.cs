...
public static class ActivityAudit
{
    ...
    public static void Add(ChatAppDbContext db, string eventType, ChatUser target, ClaimsPrincipal? actor = null,
        object? metadata = null)
        => Add(db, eventType, "User", target.Id.ToString(), target.UserName, actor, metadata);

    public static void Add(ChatAppDbContext db, string eventType, string entityType, string? entityId,
    ...
    public static void UseLoginAudit(this WebApplication app)
    {
        app.Use(async (context, next) =>
        {
            ...
            context.Response.OnStarting(async () =>
            {
                ...
                var target = context.Items[TargetKey] as ChatUser;
    ...
    public static void MapActivityLog(this WebApplication app)
    {
        app.MapGet("/api/activity-logs", async (int? page, int? pageSize, string? eventType, string? search,
            ...
            ChatAppDbContext db, HttpContext context, CancellationToken ct) =>
        {
            var number = page ?? 1;
