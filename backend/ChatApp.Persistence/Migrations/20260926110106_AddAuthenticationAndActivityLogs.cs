...
{
    /// <inheritdoc />
    public partial class AddAuthenticationAndActivityLogs : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Preserve existing chat users and initialize their authentication state.
            migrationBuilder.Sql("""
                UPDATE [Users] SET
                    [IsEnabled] = CASE WHEN [Status] = 'active' THEN 1 ELSE 0 END,
                    [LockoutEnabled] = 1,
                    [SecurityStamp] = CONVERT(varchar(64), CRYPT_GEN_RANDOM(32), 2),
                    [ConcurrencyStamp] = CONVERT(varchar(64), CRYPT_GEN_RANDOM(32), 2);
                """);
            ...
            migrationBuilder.CreateIndex(
                name: "IX_ActivityLogs_EventType_OccurredAt",
                table: "ActivityLogs",
        ...
        {
            migrationBuilder.Sql("""
                IF EXISTS (SELECT 1 FROM [Users] WHERE LEN([Username]) > 50 OR LEN([NormalizedUsername]) > 50)
                    THROW 50000, 'Cannot downgrade while user handles exceed the previous 50-character limit.', 1;
                """);
