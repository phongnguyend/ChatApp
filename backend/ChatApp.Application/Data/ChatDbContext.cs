    ...
    : DbContext(options)
{
    ...
    private static void ConfigureUserNotifications(ModelBuilder modelBuilder)
    {
        ...
        notification.ToTable("UserNotifications", table => table.HasCheckConstraint(
            "CK_UserNotifications_Type", "[Type] IN ('meeting_invite', 'meeting_rescheduled', 'meeting_cancelled', 'document_file_share', 'document_folder_share', 'note_share', 'task_share', 'task_assignment', 'message_reaction', 'recording_ready')"));
