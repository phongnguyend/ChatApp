...
const operationKey = 'chatapp-microsoft-operation'
...
export async function startMicrosoftSignIn(userId: string | null = null) {
...
export async function completeMicrosoftSignIn(tokenKey: string) {
  ...
  try {
    ...
    const headers: Record<string, string> = { Authorization: `Bearer ${result.idToken}` }
    if (linking) {
    ...
    const response = await fetch(`${API_URL}/api/auth/${linking ? 'link' : 'exchange'}/microsoft`, { method: 'POST', credentials: 'include', headers })
    if (!response.ok) {
    ...
    return { token: linking ? sessionStorage.getItem(tokenKey) : null, error: error instanceof Error ? error.message : 'Microsoft sign-in failed.' }
  } finally {
