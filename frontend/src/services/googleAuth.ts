...
const GOOGLE_STATE_KEY = 'chatapp-google-state'
const GOOGLE_NONCE_KEY = 'chatapp-google-nonce'
...
function consumeGoogleRedirect() {
  ...
  try {
    if (!token || !expectedNonce || decodeJwtPayload(token).nonce !== expectedNonce)
      return { token: null, error: 'Google sign-in nonce validation failed.' }
    return { token, error: '' }
  } catch (error) {
    return { token: null, error: error instanceof Error ? error.message : 'Google returned an invalid ID token.' }
  }
}
...
export async function completeGoogleSignIn(tokenKey: string) {
  ...
  try {
    const response = await fetch(`${API_URL}/api/auth/exchange/google`, {
      method: 'POST', credentials: 'include', headers: { Authorization: `Bearer ${redirect.token}` },
    })
  ...
  } catch (error) { return { token: null, error: error instanceof Error ? error.message : 'Unable to sign in.' } }
}
