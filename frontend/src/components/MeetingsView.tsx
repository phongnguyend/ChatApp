import { ArrowLeft, CalendarDays, CalendarX2, Check, Clock3, LoaderCircle, MessageCircle, Pencil, Plus, Search, UserRound, Users, Video, X } from "lucide-react";
...
export function MeetingsView({ apiUrl, currentUsername, onBack, onOpenConversation, hidden }: {
  ...
  return <section className="meetings-view" hidden={hidden} aria-label="Meetings">
    ...
    {selected && !form && <div className="meetings-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !cancelling && !openingConversation) closeDetails(); }}><div className="meetings-dialog" role="dialog" aria-modal="true" aria-labelledby="meetings-detail-title"><header><div><p className="eyebrow">MEETING DETAILS</p><h2 id="meetings-detail-title">{selected.title}</h2></div><button type="button" aria-label="Close meeting details" disabled={cancelling || openingConversation} onClick={closeDetails}><X size={19} /></button></header>{selected.status === "cancelled" && <p className="meetings-cancelled">Cancelled</p>}<p className="meetings-detail-date"><CalendarDays size={16} /> {dateRange(selected)} <Clock3 size={16} /> {timeRange(selected)}</p><div className="meetings-detail-section"><strong>Organizer</strong><p>{selected.organizerDisplayName} (@{selected.organizerUsername})</p></div><div className="meetings-detail-section"><strong>People</strong><p>{selected.people.length ? selected.people.map((person) => `${person.displayName} (@${person.username})`).join(", ") : "Only the organizer"}</p></div>{selected.description && <div className="meetings-detail-section"><strong>Description</strong><p>{selected.description}</p></div>}{detailLoading && <p className="meetings-muted"><LoaderCircle className="meetings-spin" size={15} /> Loading current details...</p>}{detailError && <p className="meetings-error" role="alert">{detailError}</p>}{confirmCancel && <p className="meetings-error">Cancel this meeting for everyone? It will remain visible as cancelled.</p>}<footer><button type="button" disabled={detailLoading || openingConversation || cancelling} onClick={() => void openConversation("chat")}><MessageCircle size={15} /> Chat</button>{selected.status !== "cancelled" && <button className="meetings-primary" type="button" disabled={detailLoading || openingConversation || cancelling} onClick={() => void openConversation("join")}><Video size={15} /> Join</button>}{selected.canEdit && selected.status !== "cancelled" && <><button type="button" disabled={detailLoading || cancelling} onClick={() => confirmCancel ? void cancel() : setConfirmCancel(true)}><CalendarX2 size={15} /> {confirmCancel ? "Confirm cancellation" : "Cancel meeting"}</button><button type="button" disabled={detailLoading || cancelling} onClick={() => openEdit(selected)}><Pencil size={15} /> Edit meeting</button></>}<button type="button" onClick={closeDetails}><X size={15} /> Close</button></footer></div></div>}
